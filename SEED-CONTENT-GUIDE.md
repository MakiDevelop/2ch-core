...
生成內容時，**必須確認資訊的真實性與時效性**：

1. **時事相關內容**
   - 提及新聞、事件時，先用 WebSearch 確認是否為真實事件
   - 確認事件發生時間，避免提及「過時」的新聞當作最新
   - 例：提到「最近的地震」，要確認近期是否真的有地震

2. **名人／藝人相關**
   - 確認人名、團體名稱正確
   - 確認相關事件（戀情、作品、爭議）是否真實存在
   - 避免捏造不存在的八卦

3. **產品／服務相關**
   - 確認產品、服務、App 是否真實存在
   - 確認價格、功能描述大致正確
   - 例：提到「某銀行信用卡回饋」，先查證該卡是否存在
...
5. **查詢方式**
   ```
   # 使用 WebSearch 確認
   WebSearch: "2026年1月 台灣 地震"
   WebSearch: "某某藝人 最新消息"
   WebSearch: "XX信用卡 回饋 2026"
   ```

6. **寧可模糊，不可錯誤**
   - 如果無法確認，使用模糊描述：「最近好像有...」「聽說...」
   - 或改用不涉及具體事實的主題
